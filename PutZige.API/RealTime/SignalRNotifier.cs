...
namespace PutZige.API.RealTime
{
    public class SignalRNotifier : IRealTimeNotifier
    {
        ...
        public Task TryNotifyMessageDeliveredAsync(Guid recipientUserId, Guid messageId, DateTime deliveredAt)
        {
            ...
            {
                return _hubContext.Clients.Client(connectionId).SendAsync("MessageDelivered", new { MessageId = messageId, DeliveredAt = deliveredAt });
            }
        ...
        public Task TryNotifyMessageReadAsync(Guid recipientUserId, Guid messageId, DateTime readAt)
        {
            if (_connectionMapping.TryGetConnection(recipientUserId, out var connectionId))
            {
                return _hubContext.Clients.Client(connectionId).SendAsync("MessageRead", new { MessageId = messageId, ReadAt = readAt });
            }
        ...
        {
            return _hubContext.Clients.All.SendAsync("UserOnline", new { UserId = userId, IsOnline = true, LastSeen = DateTime.UtcNow });
        }

        public Task TryNotifyUserOfflineAsync(Guid userId)
        {
            return _hubContext.Clients.All.SendAsync("UserOffline", new { UserId = userId, IsOnline = false, LastSeen = DateTime.UtcNow });
        }
        ...
        public Task TryNotifyUserTypingAsync(Guid conversationId, Guid userId)
        {
            ...
            return _hubContext.Clients.All.SendAsync("UserTyping", new { UserId = userId, ConversationId = conversationId });
        }

        public Task TryNotifyUserStoppedTypingAsync(Guid conversationId, Guid userId)
        {
            return _hubContext.Clients.All.SendAsync("UserStoppedTyping", new { UserId = userId, ConversationId = conversationId });
        }
    }
}
